...
L=[(2*alpha1+alpha2+alpha3) -(alpha1+alpha2) -(alpha1+alpha3)
   -(alpha1+alpha2)      (alpha1+alpha2)   0
   -(alpha1+alpha3)      0                 (alpha1+alpha3)];

mu12=[2;2];
mu13=[-2;2];

mu1=(alpha1+alpha2)*mu12+(alpha1+alpha3)*mu13;
mu2=-(alpha1+alpha2)*mu12;
mu3=-(alpha1+alpha3)*mu13;

W=[tf^3/3 tf^2/2
   tf^2/2 tf];

R=diag([alpha1*r alpha2*r alpha3*r]);
...
%u3=zeros(2,301);
for i=1:(10*tf+1);
u1(:,i)=-(tf-(i-1)/10)/(alpha1*r)*((alpha1+alpha2)*(s1f-s2f-mu12)+(alpha1+alpha3)*(s1f-s3f-mu13)) ...
    -1/(alpha1*r)*((alpha1+alpha2)*(v1f-v2f)+(alpha1+alpha3)*(v1f-v3f));
u2(:,i)=-(tf-(i-1)/10)/(alpha2*r)*(alpha1+alpha2)*(s2f-s1f+mu12)-1/(alpha2*r)*(alpha1+alpha2)*(v2f-v1f);
u3(:,i)=-(tf-(i-1)/10)/(alpha3*r)*(alpha1+alpha3)*(s3f-s1f+mu13)-1/(alpha3*r)*(alpha1+alpha3)*(v3f-v1f);
end
%%==============================Distributed Nash =========================%
...
% 
[the,h]=ode45(@(t,x) TerminalStateEstiPareto(t,x,g,s1,v1,s2,v2,s3,v3,mu1,mu2,mu3,tf,W,r,alpha1,alpha2,alpha3), ...
...
for i=1:(10*tf+1);
u1h(:,i)=-(tf-(i-1)/10)/(alpha1*r)*((alpha1+alpha2)*(hs1f-hs2f-mu12)+(alpha1+alpha3)*(hs1f-hs3f-mu13)) ...
    -1/(alpha1*r)*((alpha1+alpha2)*(hv1f-hv2f)+(alpha1+alpha3)*(hv1f-hv3f));
u2h(:,i)=-(tf-(i-1)/10)/(alpha2*r)*(alpha1+alpha2)*(hs2f-hs1f+mu12)-1/(alpha2*r)*(alpha1+alpha2)*(hv2f-hv1f);
u3h(:,i)=-(tf-(i-1)/10)/(alpha3*r)*(alpha1+alpha3)*(hs3f-hs1f+mu13)-1/(alpha3*r)*(alpha1+alpha3)*(hv3f-hv1f);
end



[th1,P1]=ode45(@(t,x) ControlPareto(t,x,hs1f,hs2f,hs3f,hv1f,hv2f,hv3f,mu12,mu13,tf,r,alpha1,alpha2,alpha3), ...
    [0 tf],[x1' x2' x3']);
...
[th,P]=ode45(@(t,x) ControlPareto(t,x,s1f,s2f,s3f,v1f,v2f,v3f,mu12,mu13,tf,r,alpha1,alpha2,alpha3), ...
    [0 tf],[x1' x2' x3']);
% 
...
figure;
p1=plot(P(:,1),P(:,2),'ob','LineWidth',2);hold;
plot(P(:,5),P(:,6),'ob','LineWidth',2);
plot(P(:,9),P(:,10),'ob','LineWidth',2);
i0=size(th,1);
eta0=(  norm([P(i0,1);P(i0,2)]-[P(i0,5);P(i0,6)]-mu12)+ ...
...
% eta2=(  norm([P2(i2,1);P2(i2,2)]-[P2(i2,5);P2(i2,6)]-mu12)+ ...
%             norm([P2(i2,1);P2(i2,2)]-[P2(i2,9);P2(i2,10)]-mu13))

i1=size(th1,1);
p3=plot(P1(:,1),P1(:,2),'+r','LineWidth',2);
plot(P1(:,5),P1(:,6),'+r','LineWidth',2);
plot(P1(:,9),P1(:,10),'+r','LineWidth',2);
eta1=(  norm([P1(i1,1);P1(i1,2)]-[P1(i1,5);P1(i1,6)]-mu12)+ ...
...
% xlabel('x');
% ylabel('y')
legend([p1 p3],{'Original Pareto Control','Proposed Design'})
